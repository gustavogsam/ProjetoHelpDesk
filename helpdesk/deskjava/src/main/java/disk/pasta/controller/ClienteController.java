...
public class ClienteController {
    ...
    @PostMapping("/criar")
    public ResponseEntity<?> salvar(@Valid @RequestBody ClienteRequestDTO dto){
        try {
            ...
            return ResponseEntity.status(400).body(Map.of("erro", e.getMessage(), "Sucesso", false));
        } catch (Exception e) {
            return ResponseEntity.status(500).body(Map.of("erro", "Erro ao cadastrar cliente: " + e.getMessage(), "Sucesso", false));
        }
    }
